...
<div style = "background-color: ${newPerson[0].ojos}" id='persona'>
<img src="${newPerson[0].imagen}" />
<h1>${newPerson[0].nombre}</h1>
<p>${newPerson[0].edad}</p>
<p>${newPerson[0].email}</p>
<p>${newPerson[0].direccion}</p>
</div>
`;

let seleccion = parseInt(window.prompt("Introduce tu elección: "));
if (seleccion >= 0 && seleccion < newPerson.length) {
  document.getElementById("div2").innerHTML = `
<div style = "background-color: ${newPerson[seleccion].ojos}" id='persona'>
<img src="${newPerson[seleccion].imagen}" />
<h1>${newPerson[seleccion].nombre}</h1>
<p>${newPerson[seleccion].edad}</p>
<p>${newPerson[seleccion].email}</p>
<p>${newPerson[seleccion].direccion}</p>
</div>
`;
} else {
...
let seleccion2 = window.prompt("Introduce una búsqueda: ").toLowerCase();
let encontrado = false

for(let i = 0 ; i<newPerson.length; i++){
  if(newPerson[i].nombre.includes(seleccion2)){
    encontrado = true
    parrafo += `
    ...
    parrafo = `<h1>No se ha encontrado nigún resultado con ese nombre</h1>`
  }
}
